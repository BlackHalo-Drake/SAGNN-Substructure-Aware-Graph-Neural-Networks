...
def run_test(cfg, create_dataset, test, evaluator=None):
    _, _, test_dataset = create_dataset(cfg)
    model = torch.load(cfg.train.checkpoint_path)
...
def runopt(cfg, create_dataset, create_model, train, test, evaluator=None, predict=False, predictor=None):
    ...
    val_loader = DataLoader(val_dataset, cfg.train.batch_size // cfg.sampling.batch_factor, shuffle=False,
                            num_workers=cfg.num_workers)
    test_loader = DataLoader(test_dataset, cfg.train.batch_size // cfg.sampling.batch_factor, shuffle=False,
                             num_workers=cfg.num_workers)
    ...
    for run in range(1, cfg.train.runs + 1):
        ...
        for epoch in range(1, cfg.train.epochs + 1):
            ...
            if val_perf > best_val_perf and predict is False:
                best_val_perf = val_perf
                test_perf = test(test_loader, model, evaluator=evaluator, device=cfg.device)
                ckpt_dir = os.path.join(cfg.processed_dir, "RUN_")
                if not os.path.exists(ckpt_dir):
                    os.makedirs(ckpt_dir)
                torch.save(model.state_dict(), '{}.pkl'.format(ckpt_dir + "/epoch_" + str(epoch) + "_score" + str(best_val_perf)))
            if val_perf > best_val_perf and predict is True:
                predictor(test_loader, model, evaluator=evaluator, device=cfg.device)
    ...
    return torch.abs(test_perf.mean()).numpy()


def run(cfg, create_dataset, create_model, train, test, evaluator=None, predict=False, predictor=None):
    if cfg.seed is not None:
        set_random_seed(cfg.seed)
        cfg.train.runs = 1  # no need to run same seed multiple times
    ...
    # 2. create loader
    train_loader = DataLoader(train_dataset, cfg.train.batch_size, shuffle=True, num_workers=cfg.num_workers)
    val_loader = DataLoader(val_dataset, cfg.train.batch_size, shuffle=False,
                            num_workers=cfg.num_workers)
    test_loader = DataLoader(test_dataset, cfg.train.batch_size, shuffle=False,
                             num_workers=cfg.num_workers)
    ...
    for run in range(1, cfg.train.runs + 1):
        ...
        for epoch in range(1, cfg.train.epochs + 1):
            ...
            if not os.path.exists(ckpt_dir):
                os.makedirs(ckpt_dir)
            #torch.save(model.state_dict(), '{}.pkl'.format(ckpt_dir + "/epoch_" + str(epoch) + "_score_" + str(val_perf)))
            if val_perf > best_val_perf and predict is False:
                best_val_perf = val_perf
                test_perf = test(test_loader, model, evaluator=evaluator, device=cfg.device)
        ...
        print(
            f'Run {run}, Vali: {best_val_perf}, Test: {test_perf}, Seconds/epoch: {time_average_epoch / cfg.train.epochs}, Memory Peak: {memory_allocated} MB allocated, {memory_reserved} MB reserved.')
        torch.save(model, 'model_' + str(run) + '.pt')
        test_perfs.append(test_perf)
        vali_perfs.append(best_val_perf)
...
def run_k_fold(cfg, create_dataset, create_model, train, test, evaluator=None, k=10):
    ...
    for fold, (train_idx, test_idx) in enumerate(zip(*k_fold_indices)):
        ...
        time_average_epoch = time.time() - start_outer
        print(
            f'Fold {fold}, Test: {best_test_perf}, Seconds/epoch: {time_average_epoch / cfg.train.epochs}, Memory Peak: {memory_allocated} MB allocated, {memory_reserved} MB reserved.')
