...
# TODO: maybe add some nn layer transformation after each convolution.  [test 2-layer MLP like GIN]
...
class TransformerConv(nn.Module):
    def __init__(self, nin, nout, bias=True, nhead=1):
        super().__init__()
        self.layer = gnn.TransformerConv(in_channels=nin, out_channels=nout//nhead, heads=nhead, edge_dim=nin, bias=bias)
    def reset_parameters(self):
        self.layer.reset_parameters()
    def forward(self, x, edge_index, edge_attr):
        return self.layer(x, edge_index, edge_attr)
...
class SimplifiedPNAConv(gnn.MessagePassing):
    def __init__(self, nin, nout, bias=True, aggregators=['mean', 'min', 'max', 'std'], **kwargs):
...
# TODO: implement a faster version of PNA. Call it general layer?
